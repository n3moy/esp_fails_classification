...
def curvatire_viz(PATH, wells):
    ...
    for i, well in enumerate(wells):
        ...
        if well in [4, 5, 6, 7, 8]:
            ...
            ax.plot(
                curvature["dx"], curvature["dy"], -curvature["depth_abs"], linewidth=3
            )
            ax.set_title(f"Well {i+1}")
            ax.set_zlabel("Depth, m", labelpad=7)
            plt.xticks(rotation=30)
...
def metrics_plotting(data_in: pd.DataFrame):
    well_id = data_in["well_id"].values[0]
    data = data_in.drop(["well_id", "event_id"], axis=1)
    cols = data.columns
    ...
    other = [
        i
        for i in cols
        if i not in [*current, *voltage, *pressure, *temperature, *rates]
    ]
...
def metrics_plotting_with_events(data_in: pd.DataFrame, events_in: pd.DataFrame):
    ...
    well_id = data["well_id"].values[0]
    data = data.drop(["well_id", "event_id"], axis=1)
    cols = data.columns
        ...
        for j, group_series in enumerate(group):
            ...
            for i, (ev_id, dates_list) in enumerate(events_dir.items()):
                ...
                for start_date, end_date in dates_list:
                    ...
                    msk = pd.Series(
                        (data_to_plot.index >= start_date)
                        & (data_to_plot.index <= end_date)
                    )
                    # else:
...
def feature_comparing(data_in: pd.DataFrame):
    data = data_in.copy()
    data = data.drop(["time", "well_id"], axis=1)
    cols = data.columns
